...
namespace ERP.EventManagement
{
    public class VoucherFinalCalculator
    {
        ...
        public VoucherFinalDTO VoucherCalculation( VoucherDTO voucher, List<LineItemDetails> lineitems,
              string applicableDiscountVF = "", string applicableAddChargeVF = "", VoucherTermDTO term = null)
        {
            try
            {
                ...
                taxTotal = RoundValue(taxTotal, EventVoucherSetting.TotalAmountRoundDigit);

                taxableTotal = Convert.ToDecimal(taxTrans.Sum(s => s.taxableAmount));
                taxableTotal = RoundValue(taxableTotal, EventVoucherSetting.TotalAmountRoundDigit);
                ...
                foreach (var tt in taxTrans)
                {
                    ...
                    taxTransaction.TaxAmount = RoundValue(tt.taxAmount.Value, EventVoucherSetting.TotalAmountRoundDigit);
                    taxTransaction.TaxableAmount = RoundValue(tt.taxableAmount.Value, EventVoucherSetting.TotalAmountRoundDigit);
                ...
                voucheFinal.voucher.SubTotal = RoundValue(subtotal, EventVoucherSetting.TotalAmountRoundDigit);
                decimal? discount = 0;
                ...
                voucheFinal.voucher.AddCharge = RoundValue(addtionalCharge.Value, EventVoucherSetting.TotalAmountRoundDigit);
                voucheFinal.voucher.Discount = RoundValue(discount.Value, EventVoucherSetting.TotalAmountRoundDigit); 


                //  taxableTotal += (decimal)(addtionalCharge - discount);
                voucheFinal.voucher.GrandTotal = RoundValue((taxableTotal + taxTotal), EventVoucherSetting.TotalAmountRoundDigit);
