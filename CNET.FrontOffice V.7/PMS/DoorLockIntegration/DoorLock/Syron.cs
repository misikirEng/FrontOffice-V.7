...
using CNET.FrontOffice_V._7.PMS.DoorLockIntegration.DoorLock.SyronReader;
...
namespace CNET.FrontOffice_V._7.PMS.DoorLockIntegration.DoorLock
{
    ...
    internal class SerialPortReader : ReaderBase
    {
        ...
        public int MacInitialization()
        {
            ...
            {
               // MessageBox.Show("Checking Port : " + text);
                try
                {
                    ...
                    if (mi != null)
                    {
                        //MessageBox.Show("Found PortName " + text);
                        PortSetting.SetConfiguration("PortName", text);
                        return 1;
                    }
                    else
                    {
                       // MessageBox.Show("Null Port " + text);
                        SerialPort.Close();
                    }
                }
                catch (Exception)
        ...
        List<byte> ll = new List<byte> { 235,99,40,86,0,65,122,148 ,183 ,0   ,0   ,0   ,0   ,
0   ,
24  ,
6   ,
5   ,
            ...
            try
            {
                ...
                lock (SerialPort)
                {
                    ...
                    if (b == ReturnIdentification.ReadSuccess)
                    {
                        ...
                        //string va = "";
                        //int cou = 1;
                        //foreach(byte bv in CurrentResultBys)
                        //{
                        //    va += cou+". "+ bv + Environment.NewLine;
                        //    cou++;
                        //}

                        //MessageBox.Show("Data:- " + Environment.NewLine+ va);
                        rci.TransToData(CurrentResultBys);



                        // MessageBox.Show("return 1");
                        return 1;
                    }
                }
            }
