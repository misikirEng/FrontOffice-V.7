...
{
    public class DelunsLock : IDoorLock
    {
        ...
        public CardInfo ReadCardData(bool showStatusMessage = true)
        {
            ...
            try
            {
                ...
                if (st == 0)
                {
                    ...
                    //Read Lock Number 
                    StringBuilder lockNumber = new StringBuilder(8);
        ...
        public bool IssueGuestCard(string lockNumber, DateTime startDate, DateTime endDate, bool isDuplicate = false)
        {
            try
            {
                ...
                if (lockNumber.Length != 8)
                {
                    XtraMessageBox.Show("Length of lock number must be 8.", "Door Lock", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
                }
                ...
                StringBuilder sbLock = new StringBuilder(8);
        ...
        [DllImport(@"lib\deluns\Delus V09\proRFL.dll", EntryPoint = "GetDLLVersion")]
        public static extern int GetDLLVersion(StringBuilder buffer);

        [DllImport(@"lib\deluns\Delus V09\proRFL.dll", EntryPoint = "initializeUSB")]
        public static extern int initializeUSB(short fUSB);

        [DllImport(@"lib\deluns\Delus V09\proRFL.dll", EntryPoint = "Buzzer")]
        public static extern int Buzzer(short fUSB, int t);

        [DllImport(@"lib\deluns\Delus V09\proRFL.dll", EntryPoint = "ReadCard")]
        public static extern int ReadCard(short fUSB, StringBuilder buffData);

        [DllImport(@"lib\deluns\Delus V09\proRFL.dll", EntryPoint = "GuestCard")]
        ...
        [DllImport(@"lib\deluns\Delus V09\proRFL.dll", EntryPoint = "CardErase")]
        public static extern int CardErase(short fUSB, int dlsCoID, StringBuilder cardHexStr);

        [DllImport(@"lib\deluns\Delus V09\proRFL.dll", EntryPoint = "GetCardTypeByCardDataStr")]
        public static extern int GetCardTypeByCardDataStr(StringBuilder cardDataStr, StringBuilder cardType);

        [DllImport(@"lib\deluns\Delus V09\proRFL.dll", EntryPoint = "GetGuestLockNoByCardDataStr")]
        public static extern int GetGuestLockNoByCardDataStr(int dlsColID, StringBuilder cardDataStr, StringBuilder lockNumber);

        [DllImport(@"lib\deluns\Delus V09\proRFL.dll", EntryPoint = "GetGuestETimeByCardDataStr")]
