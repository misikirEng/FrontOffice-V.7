...
namespace CNET.FrontOffice_V._7.Night_Audit
{
    public partial class frmAuditNightAll :  UILogicBase
    {
       ...
       // private string reportArchiveConfig, reportArchivePrint;
      ...
      //  private BackgroundWorker bWorker = null;
        ...
        public bool InitializeData()
        {
            try
            {
                ...
                List<PeriodDTO> PeriodList = UIProcessManager.GetPeriodByType(CNETConstantes.PERIOD_TYPE_PMS);
                ...
                GetHotelandData();
                _frmReport = new frmPMSReports(true, HomePage.dashboard);
                try
                {
                ...
                List<ConfigurationDTO> pmsSettings = LocalBuffer.LocalBuffer.ConfigurationBufferList.Where(c => c.Reference == CNETConstantes.PMS_Pointer.ToString()).ToList();
                ...
                organazationUnitDefn = LocalBuffer.LocalBuffer.CurrentDeviceConsigneeUnit.Value;
                ...
                if (CurrentDevice != null)
                {
                    ...
                    id = UIProcessManager.IdGenerater("Voucher", CNETConstantes.CASHSALESSUMMRY, 0, LocalBuffer.LocalBuffer.CurrentDeviceConsigneeUnit.Value, false, CurrentDevice.Id);
                }
                if (id != null)
                {
                    teVoucherNo.Text = id;
                ...
                /* ConfigurationDTO isJournalizationEnabledConfig = LocalBuffer.LocalBuffer.ConfigurationBufferList.FirstOrDefault(c => c.Reference == CNETConstantes.PMS_Pointer.ToString() && c.Attribute == CNETConstantes.PMS_SETTING_EnableJournalization);
                 ConfigurationDTO postRoutConfig = LocalBuffer.LocalBuffer.ConfigurationBufferList.FirstOrDefault(c => c.Reference == CNETConstantes.PMS_Pointer.ToString() && c.Attribute == CNETConstantes.PMS_SETTING_PostineRoutine);
        ...
        public void GetHotelandData()
        {
            ...
            CurrentDeviceOrganizationUnit = LocalBuffer.LocalBuffer.CurrentDeviceConsigneeUnit;
        ...
        {
            AuditPeriodwithNoAuditdoneList = new List<PeriodDTO>();
            DateTime? TodayDate = UIProcessManager.GetServiceTime();
            ClosingValidationDTO LastClosingValidation = UIProcessManager.SelectClosingValidationByComponentandBranch(CNETConstantes.PMS_Pointer, SelectedHotelcode.Value);

            if (LastClosingValidation != null)
            {
                PeriodDTO Periods = new PeriodDTO();
                PeriodDTO PeriodLast = LocalBuffer.LocalBuffer.PeriodBufferList.FirstOrDefault(x => x.Id == LastClosingValidation.Period);
                ...
                {
                    Periods = LocalBuffer.LocalBuffer.PeriodBufferList.FirstOrDefault(x => x.Start.Date == date.Date && x.Type == CNETConstantes.PERIOD_TYPE_PMS);
                    if (Periods == null)
                    {
            ...
            {
                PeriodDTO PeriodList = UIProcessManager.GetPeriodByDateAndType(TodayDate.Value.Date, CNETConstantes.PERIOD_TYPE_PMS);
                if (PeriodList == null)
                {
        ...
        private bool PerfomCheckout(RegistrationListVMDTO regExten, bool isWithoutRecept, bool isWithZeroBalance = false, bool isReinstate = false)
        {
            ...
            try
            {
                ...
                var osd = LocalBuffer.LocalBuffer.ObjectStateDefinitionBufferList.FirstOrDefault(o => o.Id == vo.Voucher.LastState);
        ...
        private bool ChangeAllRoomsState()
        {
            ...
            try
            {
               // Progress_Reporter.Show_Progress("Changing Room Status..");
                ...
                if (rmDetails != null)
                {
                    //Progress_Reporter.Show_Progress("Updating rooms status to dirty", rmDetails.Count);
        ...
        private void LoadReportArchiveConfig()
        {
           ...
           // reportArchiveConfig = Environment.CurrentDirectory + @"//PMS Report";
           ...
           /* if (CurrentDevice != null)
            ...
            //Default printer
            List<ConfigurationDTO> regVocConfigs = LocalBuffer.LocalBuffer.ConfigurationBufferList.Where(c => c.Reference == CNETConstantes.REGISTRATION_VOUCHER.ToString()).ToList();
        ...
        private void PopulateOverDueOuts()
        {
            ...
            if (_regListVM != null)
            {
                ...
                gcOverDueOut.DataSource = filtered == null ? new List<RegistrationListVMDTO>() : filtered;
        ...
        private void PopulateDueOuts()
        {
            ...
            gcDueOut.DataSource = _regListVM == null ? new List<RegistrationListVMDTO>(): _regListVM;
        ...
        private void PopulateWaitings()
        {
            ...
            gcWaitinglist.DataSource = _regListVM == null ? new List<RegistrationListVMDTO>() : _regListVM;
        ...
        private void PopulateSixPMs()
        {
            ...
            gc6PM.DataSource = _regListVM == null ? new List<RegistrationListVMDTO>() : _regListVM;
            gv6PM.RefreshData();
        }
        ...
        private void PopulateGuranteed()
        {
            ...
            gcGauranted.DataSource = _regListVM == null ? new List<RegistrationListVMDTO>() : _regListVM;
        ...
        private void PopulateHeldBills()
        {
            ...
            gcClearHeldBills.DataSource = filtered == null ? new List<RegistrationListVMDTO>() : filtered;
        ...
        private void PopulateRoomAndTax()
        {
            ...
            foreach (DailyRoomChargeDTO dr in dailyRoomChargeList)
            {
               // RegistrationDetail rd = UIProcessManager.GetRegistrationDetailByVoucher(dr.registrationVoucher).FirstOrDefault();
                ...
                ConsigneeDTO per = new ConsigneeDTO();
                if (dr.dailyRoomChargeVoucher != null)
                {
                    per = LocalBuffer.LocalBuffer.ConsigneeViewBufferList.FirstOrDefault(p => p.Id == dr.dailyRoomChargeVoucher.Consignee1);
                }
                ...
                if (dr.dailyRoomChargeVoucher != null) dto.amount = Math.Round(dr.dailyRoomChargeVoucher.GrandTotal == null ? 0 : dr.dailyRoomChargeVoucher.GrandTotal, 2);
        ...
        private void PopulateBucketCheck()
        {
            ...
            foreach (RegistrationListVMDTO regDoc in _regListVM)
            {
                ...
                if (regDoc.RateCodeHeader != null)
                {
                    ...
                    {
                        currencyDescription = LocalBuffer.LocalBuffer.CurrencyBufferList.FirstOrDefault(x => x.Id == Header.CurrencyCode).Description;
                    }
                }
        ...
        private void PopulateReportTree()
        {
            ...
            List<ReportDTO> filtered = reportList.Where(r =>  r.Reference == CNETConstantes.Report_Category_Night_Audit && r.IsActive == true).ToList();
            ReportDTO DashboardReport = new ReportDTO
            {
                Id = 0,
                Index=0, 
                Subsystem = CNETConstantes.Report_Category_Night_Audit,
                 ...
                 List<string> RoomtypeDescList = _roomTypeList.Select(x => x.Description).ToList(); ;
                  ...
                  foreach (DiscrepancyDTO discripancy in filtered)
                  {
                      ...
                      if (RoomtypeDescList.Contains(rr.RoomType))
                      {
                          ...
                          LookupDTO look = LocalBuffer.LocalBuffer.LookupBufferList.FirstOrDefault(l => l.code == discripancy.hkValue);
                          if (look != null)
                          {
        ...
        {
            treeListReports.PostEditor();
            List<TreeListNode> nodeList = treeListReports.GetAllCheckedNodes();

            List<ReportDTO> check = (List<ReportDTO>) treeListReports.DataSource;
            ...
            TreeListColumn colCode = treeListReports.Columns["Id"];
            ...
            foreach (TreeListNode node in nodeList)
            {
                ...
                var vv = node.GetValue(colCode);
                pathHolder.Code = vv.ToString();
        ...
        private void PopulateSalesSummary()
        {
            try
            {
                ...
                var config = LocalBuffer.LocalBuffer.ConfigurationBufferList.FirstOrDefault(c => c.Reference == CNETConstantes.CASHSALESSUMMRY.ToString() && c.Attribute.ToLower() == "Round Digit Unit Price");
                if (config != null)
                    _roundSalesSummary = Convert.ToInt32(config.CurrentValue);

                Progress_Reporter.Show_Progress("Populating sales summary", "Please Wait...");
                ...
                decimal grandTotal = _salesSummaryList.Sum(s =>s.totalAmt ==null ? 0:  s.totalAmt);
                ...
                foreach (var sale in _salesSummaryList)
                {
                    SalesSummaryVM dto = new SalesSummaryVM()
                    {
                        ...
                        Quantitiy = Math.Round(sale.totalQuantity, _roundSalesSummary),
                        TotalAmount = Math.Round( sale.totalAmt, _roundSalesSummary),
                        UnitAmount = Math.Round(sale.UnitAmt, 2)
                    };
        ...
        private void PopulateOrderStations()
        {
            try
            {
                Progress_Reporter.Show_Progress("Reading unique order station Map .... ", "Please Wait.......");
                ...
                Progress_Reporter.Show_Progress("Reading All Product Consumption.... ", "Please Wait.......");
        ...
        private void GetSataionDefaultStore()
        {
            ...
            var stationSettings = LocalBuffer.LocalBuffer.ConfigurationBufferList.Where(x=> x.Reference == Station.ToString()).ToList();
            ...
            try
            {
                ...
                DialogResult result =
                    XtraMessageBox.Show("Are you sure to save row material consamption voucher?",
                        "CNET ERP", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation);
                ...
                if (Station != null)
                {
                    ...
                    Progress_Reporter.Show_Progress("Reading Order Station Store Mapping .... ", "Please Wait.......");
                    ...
                    Progress_Reporter.Show_Progress("Reading Current Id From Database .... ", "Please Wait.......");

                    string idd = UIProcessManager.IdGenerater("Voucher",CNETConstantes.ROW_MATERIAL_CONSUMPTION_VOUCHER,1,LocalBuffer.LocalBuffer.CurrentDeviceConsigneeUnit.Value,false, LocalBuffer.LocalBuffer.CurrentDevice.Id);
                    ...
                    {
                        Progress_Reporter.Show_Progress("Extracting Line Items From Order station map list .... ", "Please Wait.......");
                        ...
                        foreach (var orders in OrderStationConsumptionByStion)
                        {
                            foreach (var order in orders.consumptionSummary)
                            {
                                ...
                                lineItemBuff.LineItem.ObjectState = null;
                        ...
                        Progress_Reporter.Show_Progress("Checking Voucher Id Exixtance.... ", "Please Wait.......");
                        //var voucherIsExist = UIProcessManager.GetvoucheryId(CurrentVoucherNo);
                        //if (voucherIsExist != null)
                        //{
                        //    XtraMessageBox.Show("Voucher No:-" + CurrentVoucherNo + " is exist", "CNET ERP",
                        ...
                        voucherBuffer.Voucher.LastState = _rowConsumptionWorkflow.State.Value;
...
*/
        ...
        private void gvFandBVouchers_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            try
            {
                ...
                Progress_Reporter.Show_Progress("Assigning order station .... ", "Please Wait.......");
                ...
                if (station != null)
                {
                    ...
                    {
                        Progress_Reporter.Show_Progress("Reading assiged Product for each station", "Please Wait.......");
                    ...
                    {
                        Progress_Reporter.Show_Progress("Reading assiged Product for station", "Please Wait.......");
        ...
        private void bbiSave_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                ...
                else if (wcNightAudit.SelectedPage == wpSummarizeTransaction)
                {
                    ...
                    VoucherBuffer VoucherBuffer = new VoucherBuffer();
                    String id = null;
                    id = UIProcessManager.IdGenerater("Voucher",CNETConstantes.CASHSALESSUMMRY,1,LocalBuffer.LocalBuffer.CurrentDeviceConsigneeUnit.Value,false,LocalBuffer.LocalBuffer.CurrentDevice.Id);
                    ...
                    Progress_Reporter.Show_Progress("Saving Sales Summary", "Please Wait...");
                    ...
                    VoucherBuffer.Voucher.LastState = _salesSummaryWorkflow.State.Value;
                    ...
                    foreach (var sales in _salesSummaryList)
                    {
                        ...
                        lineItemBuffer.LineItem = new LineItemDTO()
                        {
                            ...
                            Quantity = sales.totalQuantity == null ? 0 : sales.totalQuantity,
                        ...
                        if (sales.discount != null && sales.discount.Value > 0 && sales.serviceCharge != null && sales.serviceCharge.Value > 0)
                        {
                            ...
                            if (sales.discount != null && sales.discount.Value > 0)
                            {
                                ...
                                if (art != null && art.Preference == LocalBuffer.LocalBuffer.ACCOMODATION_PREFERENCE_CODE && _roomDiscount >0)
                                {
                    ...
                    VoucherBuffer.Voucher.AddCharge = _salesSummaryList.Sum(s => s.serviceCharge == null ? 0 : s.serviceCharge.Value);
                    ...
                    {
                        SystemMessage.ShowModalInfoMessage("Summary is not saved!"+Environment.NewLine+ isSaved.Message, "ERROR");
                    }
        ...
        private bool IsBucketChecked(int regId, DateTime date)
        {
             ...
             {
                ActivityDTO activity = activityList.FirstOrDefault(a => a.ActivityDefinition == adBucketCheck.Id && a.TimeStamp.Date == date.Date);
                 if (activity == null)
                     return false;
                 else
                     return true;
        ...
        private void wcNightAudit_NextClick(object sender, DevExpress.XtraWizard.WizardCommandButtonClickEventArgs e)
        {
            ...
            if (e.Page == wpBucketCheck)
            {
                ...
                if (selectedRows != null && selectedRows.Length > 0)
                {
                    ...
                    Progress_Reporter.Show_Progress("Saving activity", "Please Wait...");
                    try
                    {
        ...
        private void bbiCheckOut_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            if (selectedReg != null)
            {
               // List<VwDailyChargePostingViewDTO> charges = UIProcessManager.GetCheckOutDetailViewByVoucher(selectedReg.Id, CNETConstantes.DAILY_ROOM_CHARGE_VOUCHER, null);
        ...
        private void bbiCheckIn_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            if (selectedReg != null)
            {
                ...
                {
                    var osdBuffer = LocalBuffer.LocalBuffer.ObjectStateDefinitionBufferList.FirstOrDefault(osd => osd.Id == selectedReg.lastState);
        ...
        private void bbiCancel_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            if (selectedReg != null)
            {
                ...
                if (dr == DialogResult.Yes)
                {
                    var response = UIProcessManager.GetVoucherBufferById(selectedReg.Id); 
        ...
        private void bbiPush_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            try
            {
                if (heldTran != null)
                {
                    ...
                    VoucherBuffer voucherbuffer = response.Data;
                    var pushWorkflow = UIProcessManager.GetActivityDefinitionBydescriptionandreference(CNETConstantes.LU_ACTIVITY_DEFINATION_PUSHED, voucherbuffer.Voucher.Definition).FirstOrDefault();
                    var voDef = LocalBuffer.LocalBuffer.SystemConstantDTOBufferList.FirstOrDefault(a => a.Id == voucherbuffer.Voucher.Definition);
                    ...
                    if (actList.Count == 0)
                    {
                        ...
                        //voucherbuffer.Voucher.LastState = pushWorkflow.State.Value;
        ...
        private void bbiVoid_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            if (selected != null)
            {
                ...
                try
                {
                    ...
                    var response = UIProcessManager.GetVoucherBufferById(selected.VoucherId);
                    ...
                    var voidWorkflow = UIProcessManager.GetActivityDefinitionBydescriptionandreference(CNETConstantes.LU_ACTIVITY_DEFINATION_VOID, voucherbuffer.Voucher.Definition).FirstOrDefault();
                    ...
                    voucherbuffer.Voucher.LastState = voidWorkflow.State.Value;
        ...
        private void wcNightAudit_SelectedPageChanged(object sender, DevExpress.XtraWizard.WizardPageChangedEventArgs e)
        {
            ...
            else if (e.Page == completionWizardPage1)
            {
                ...
                int period = Convert.ToInt32(sleAuditDate.EditValue.ToString());

                ClosingValidationDTO val = new ClosingValidationDTO
                {
                    SystemConstant = CNETConstantes.PMS_Pointer,
                    Period = period,
                    ...
                    Status = SelectedHotelcode.Value,
                     //= SelectedHotelcode

                };
                UIProcessManager.CreateClosingValidation(val);

                Progress_Reporter.Show_Progress("Changing room status...", "Please Wait.......");
                ...
                Progress_Reporter.Show_Progress("Incrementing current date...", "Please Wait.......");
        ...
        private void bbiPostAll_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            //OrganizationUnit HotelOrgunit = LocalBuffer.LocalBuffer.OrganizationUnitBufferList.FirstOrDefault(x=> x.reference == dailyRoomChargeList.reg);
        ...
        private void ArchiveReport()
        {
            ...
            int counter = 0;
            int lastPerc = 0;
            //string baseDir = GetExportPath(reportArchiveConfig);
            UserStateHolder userState = new UserStateHolder();
            foreach (ReportPathHolder pHolder in pathList)
            {
                if (pHolder.Code != "0")
                {
                    ...
                    try
                    {
                        ...
                        _frmReport.SelectedHotelcode = SelectedHotelcode;
                        _frmReport.ClearGrid();
                        _frmReport.ReportShow(CurrentTime, pHolder.Name);
                        ...
                        if (PDFstream != null)
                        {
                            ...
                            if (Exist)
                            {
                                ...
                                string FTPLocation = FTPInterface.FTPAttachment.SendPMSReportAttachementStream(PDFstream, pHolder.Name, CurrentTime);
                            }  
                ...
                else
                {
                    ...
                    if (PDFstream != null)
                    {
                        ...
                        {
                            FTPInterface.FTPAttachment.ORGUnitDefcode = SelectedHotelcode.ToString();
                            string FTPLocation = FTPInterface.FTPAttachment.SendPMSReportAttachementStream(PDFstream, "Dash Board Report", CurrentTime);
                        }
                    }
        ...
        private void UpdateArchiveStatus(string reportCode, string status)
        {
            ...
            ReportDTO report = allReports.FirstOrDefault(r => r.Id.ToString() == reportCode);
        ...
        private void dockPanel_attachments_Expanding(object sender, DevExpress.XtraBars.Docking.DockPanelCancelEventArgs e)
        {
            ...
            //    List<Lookup> lookups = LocalBuffer.LocalBuffer.LookupBufferList.Where(l => l.type.ToLower() == "attachment catagory").ToList();
            //    if (lookups != null)
            //    {
        ...
        private void sleAuditDate_EditValueChanged(object sender, EventArgs e)
        {
            ...
            {
                PeriodDTO selectedPeriod = UIProcessManager.GetPeriodById( Convert.ToInt32(sleAuditDate.EditValue));
        ...
        private void sleAuditDate_CustomDisplayText(object sender, CustomDisplayTextEventArgs e)
        {
            ...
            if (e.DisplayText != "")
            {
              PeriodDTO  Periods = UIProcessManager.GetPeriodById((int)e.Value);
        ...
        public List<SalesSummarized> GetSalesSummaryByDate(DateTime CurrentTime, int SelectedHotelcode)
        {
            ...
            if (SalesFilter != null && SalesFilter.Count > 0)
            {
                ...
                SalesSummarized()
                {
                    ...
                    serviceCharge = g.Sum(u => u.AddCharge)
                }).ToList();
            }
